...
export function DailyChart({ dailyStats, periodLabel }: Props) {
  ...
  const data = dailyStats.map((d) => ({
    ...d,
    appoRate: d.count > 0 ? Math.round((d.appoCount / d.count) * 1000) / 10 : null as number | null,
  }));

  return (
    <section className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
      <h2 className="text-xl font-semibold text-zinc-900">{title}</h2>
      <p className="mt-1 text-sm text-zinc-500">架電数は棒、アポ率は線で表示（ツールチップでアポ数も確認できます）</p>
          ...
          >
            ...
            <Tooltip
              content={({ active, payload, label }) => {
                ...
                const rate = row && row.count > 0 ? Math.round((row.appoCount / row.count) * 1000) / 10 : null;
                return (
            ...
            <Bar
              ...
              maxBarSize={40}
            >
              <LabelList dataKey="count" position="top" fill="#18181b" fontSize={14} fontWeight={700} />
            </Bar>
            <Line
              yAxisId="right"
              type="monotone"
